...
class Rukun_tetangga extends Backend_Controller {
	
	public $mod = 'rukun-tetangga';

	public function __construct() 
	{
		parent::__construct();
		
		$this->load->model("mod/rukun_tetangga_model");
		$this->meta_title("Rukun Tetangga");
	}
		...
		if ($this->role->i('read'))
		{
			if ($this->input->is_ajax_request()) 
			{
				...
				else
				{
					...
					foreach ($this->rukun_tetangga_model->datatable('_data', 'data') as $val) 
					{
						$row = [];
						// checkbox (all row)
					...
					$this->json_output(['data' => $data, 'recordsFiltered' => $this->rukun_tetangga_model->datatable('_data', 'count')]);
				}
			...
			if ($this->input->method() == 'post')
			{
				...
				if ($this->form_validation->run())
				{
					$data_isert = array(
						...
						'ert' => xss_filter($this->input->post('ert')),
						'kode_rt' => xss_filter($this->input->post('kode_rt')),
					);

					if ($this->rukun_tetangga_model->insert($data_isert))
					{
	...
	public function edit($id_data = '')
	{
		if ($this->role->i('modify'))
		{
			...
			$cek_id = $this->rukun_tetangga_model->cek_id($id_edit);
			...
			if ($cek_id == 1) 
			{
				if ($this->input->method() == 'post')
				{
					...
					if ( $this->form_validation->run() )
					{
						...
						if ($this->rukun_tetangga_model->update($id_edit, $data_update))
						{
				...
				$data_edit = $this->rukun_tetangga_model->get_data_edit($id_edit);
		...
		if ($this->input->is_ajax_request())
		{
			if ($this->role->i('delete'))
			{
				...
				foreach ($data as $key)
				{
					...
					$this->rukun_tetangga_model->delete($pk);
				}
