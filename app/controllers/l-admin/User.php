...
class User extends Backend_Controller {
	...
	protected $path_photo = PUBLICPATH.'uploads/user/';
		...
		if ($param == 'add') 
		{
			if ( $this->input->is_ajax_request() && $this->role->i('write'))
			{
				$this->form_validation->set_rules(array(
					...
					array(
						'field' => 'provinsi',
						'label' => lang_line('_prov'),
						'rules' => 'max_length[20]',
					),
					array(
						'field' => 'kabupaten',
						'label' => lang_line('_kab'),
						'rules' => 'max_length[20]',
					),
					array(
						'field' => 'kecamatan',
						'label' => lang_line('_kec'),
						'rules' => 'max_length[20]',
					),
					array(
						'field' => 'desa',
						'label' => lang_line('_desa'),
						'rules' => 'max_length[20]',
					),
					array(
						'field' => 'rw',
				...
				if ( $this->form_validation->run() ) 
				{
					...
					$data = array(
						...
						'provinsi'  => xss_filter($this->input->post('provinsi'),'xss'),
						'kabupaten'  => xss_filter($this->input->post('kabupaten'),'xss'),
						'kecamatan'  => xss_filter($this->input->post('kecamatan'),'xss'),
						'desa'  => xss_filter($this->input->post('desa'),'xss'),
						'rw'  => xss_filter($this->input->post('rw'),'xss'),
						'rt'  => xss_filter($this->input->post('rt'),'xss'),						
