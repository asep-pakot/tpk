...
class Remaja extends Backend_Controller {
	...
	public function __construct() 
	{
		...
		$this->load->model("mod/remaja_model");
		$this->meta_title("Remaja");
	}
		...
		if ($this->role->i('read'))
		{
			if ($this->input->is_ajax_request()) 
			{
				...
				else
				{
					...
					foreach ($this->remaja_model->datatable('_data', 'data') as $val) 
					{
						$row = [];
						// checkbox (all row)
					...
					$this->json_output(['data' => $data, 'recordsFiltered' => $this->remaja_model->datatable('_data', 'count')]);
				}
			...
			if ($this->input->method() == 'post')
			{
				...
				if ($this->form_validation->run())
				{
					$data_isert = array(
						...
						'nama_remaja' => xss_filter($this->input->post('nama_remaja')),
						'gender' => xss_filter($this->input->post('gender')),
						'tgl_lahir' => xss_filter($this->input->post('tgl_lahir')),
						'usia' => xss_filter($this->input->post('usia')),
						'no_wa' => xss_filter($this->input->post('no_wa')),
						'ikut_pik' => xss_filter($this->input->post('ikut_pik')),
						'materi' => xss_filter($this->input->post('materi')),
						'photo' => xss_filter($this->input->post('photo')),
					);

					if ($this->remaja_model->insert($data_isert))
					{
	...
	public function edit($id_data = '')
	{
		if ($this->role->i('modify'))
		{
			...
			$cek_id = $this->remaja_model->cek_id($id_edit);
			...
			if ($cek_id == 1) 
			{
				if ($this->input->method() == 'post')
				{
					...
					if ( $this->form_validation->run() )
					{
						...
						if ($this->remaja_model->update($id_edit, $data_update))
						{
				...
				$data_edit = $this->remaja_model->get_data_edit($id_edit);
