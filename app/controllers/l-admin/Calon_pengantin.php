...
class Calon_pengantin extends Backend_Controller {
	
	public $mod = 'calon-pengantin';

	public function __construct() 
	{
		parent::__construct();
		
		$this->load->model("mod/calon_pengantin_model");
		$this->meta_title("Calon Pengantin");
	}
		...
		if ($this->role->i('read'))
		{
			if ($this->input->is_ajax_request()) 
			{
				...
				else
				{
					...
					foreach ($this->calon_pengantin_model->datatable('_data', 'data') as $val) 
					{
						$row = [];
						// checkbox (all row)
					...
					$this->json_output(['data' => $data, 'recordsFiltered' => $this->calon_pengantin_model->datatable('_data', 'count')]);
				}
			...
			if ($this->input->method() == 'post')
			{
				...
				if ($this->form_validation->run())
				{
					$data_isert = array(
						...
						'nama_catin' => xss_filter($this->input->post('nama_catin')),
						'no_wa' => xss_filter($this->input->post('no_wa')),
						'tgl_akad' => xss_filter($this->input->post('tgl_akad')),
						'dok_na' => xss_filter($this->input->post('dok_na')),
						'ttd' => xss_filter($this->input->post('ttd')),
						'elsimil' => xss_filter($this->input->post('elsimil')),
						'materi' => xss_filter($this->input->post('materi')),
						'photo' => xss_filter($this->input->post('photo')),
					);

					if ($this->calon_pengantin_model->insert($data_isert))
					{
	...
	public function edit($id_data = '')
	{
		if ($this->role->i('modify'))
		{
			...
			$cek_id = $this->calon_pengantin_model->cek_id($id_edit);
			...
			if ($cek_id == 1) 
			{
				if ($this->input->method() == 'post')
				{
					...
					if ( $this->form_validation->run() )
					{
						...
						if ($this->calon_pengantin_model->update($id_edit, $data_update))
						{
				...
				$data_edit = $this->calon_pengantin_model->get_data_edit($id_edit);
		...
		if ($this->input->is_ajax_request())
		{
			if ($this->role->i('delete'))
			{
				...
				foreach ($data as $key)
				{
					...
					$this->calon_pengantin_model->delete($pk);
				}
